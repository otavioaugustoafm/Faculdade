...
public class tp2q13 {
    public static void main(String[] args) {
        ...
        // Algoritmo de ordenação Merge Sort usando types como critério primário
        pokemonArray = mergeSort(pokemonArray);
        ...
        try (PrintWriter writer = new PrintWriter("851568_mergesort.txt")) {
    ...
    // Método de ordenação Merge Sort adaptado para ordenar por types
    private static Pokemon[] mergeSort(Pokemon[] array) {
        if (array.length <= 1) {
            return array;
        }

        int mid = array.length / 2;
        Pokemon[] left = Arrays.copyOfRange(array, 0, mid);
        Pokemon[] right = Arrays.copyOfRange(array, mid, array.length);
    ...
    private static Pokemon[] merge(Pokemon[] left, Pokemon[] right) {
        Pokemon[] merged = new Pokemon[left.length + right.length];
        int leftIndex = 0, rightIndex = 0, mergeIndex = 0;

        while (leftIndex < left.length && rightIndex < right.length) {
            // Comparar tipos e usar o nome como critério secundário em caso de empate
            int comparison = comparePokemons(left[leftIndex], right[rightIndex]);
            if (comparison <= 0) {
    ...
    // Método para comparar dois Pokémons por types e, em caso de empate, pelo nome
    private static int comparePokemons(Pokemon p1, Pokemon p2) {
        // Comparar pelo primeiro tipo na lista de tipos (se existir)
        if (!p1.getTypes().isEmpty() && !p2.getTypes().isEmpty()) {
            int typeComparison = p1.getTypes().get(0).compareToIgnoreCase(p2.getTypes().get(0));
            if (typeComparison != 0) {
                return typeComparison;
            }
        } else if (p1.getTypes().isEmpty() && !p2.getTypes().isEmpty()) {
            return 1; // Coloca p2 antes de p1
        } else if (!p1.getTypes().isEmpty() && p2.getTypes().isEmpty()) {
            return -1; // Coloca p1 antes de p2
        }

        // Caso os tipos sejam iguais ou não existam, comparar pelo nome
