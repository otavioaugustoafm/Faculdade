...
public class tp2q03 {
    public static void main(String[] args) {
        ...
        List<Pokemon> using2 = new ArrayList<>();
        ...
        // Ler até FIM, procurar por nome, e adicionar na List
        while (!(input_string = sc.nextLine()).equals("FIM")) {
            Pokemon found = (PokemonSearch.searchPokemonName(pokemons, input_string));
            numComparacoes++; // Incrementar número de comparações
            if (found != null) {
                using2.add(found);
            }
        ...
        // Printar instancia por instancia da List e contar as comparações durante a pesquisa
        int maxIterations = Math.min(using.size(), using2.size());
        for (int i = 0; i < maxIterations; i++) {
            Pokemon p = using.get(i);
            boolean resp = false;

            for (Pokemon j : using2) {
                numComparacoes++; // Incrementar número de comparações para cada tentativa de comparação
                if (p.equals(j)) {
                    resp = true;
                    break;
                }
        ...
        try (PrintWriter writer = new PrintWriter("851568_sequencial.txt")) {
            writer.printf("851568\t%dms\t%d\n", executionTime, numComparacoes);
        } catch (IOException e) {
            e.printStackTrace();
        }
