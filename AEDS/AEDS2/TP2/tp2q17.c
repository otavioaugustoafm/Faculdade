...
void heapify(Pokemon *pokemons[], int n, int i) {
    ...
    if (esquerda < n && 
        (pokemons[esquerda]->altura > pokemons[maior]->altura || 
        (pokemons[esquerda]->altura == pokemons[maior]->altura && strcmp(pokemons[esquerda]->nome, pokemons[maior]->nome) > 0))) {
        maior = esquerda;
    ...
    if (direita < n && 
        (pokemons[direita]->altura > pokemons[maior]->altura || 
        (pokemons[direita]->altura == pokemons[maior]->altura && strcmp(pokemons[direita]->nome, pokemons[maior]->nome) > 0))) {
        maior = direita;
        comparacoes = comparacoes + 3;
    }
...
void heapsort(Pokemon *pokemons[], int n) {
...
{   
    ...
    heapsort(pokemon_id,cont);

    for (int i = 0; i < 10 && i < sizeof(pokemon_id); i++) {
        imprimir_pokemon(pokemon_id[i]); 
    }

    fim = clock();
    double tempo_execucao = (double)(fim - inicio) / CLOCKS_PER_SEC;

    FILE *log_file = fopen("851568_heapsortParcial.txt", "w");
    if (log_file) {
        fprintf(log_file, "%s\t%d\t%d\t%lf\n", "851568", comparacoes, movimentacoes, tempo_execucao);
        fclose(log_file);
    }
