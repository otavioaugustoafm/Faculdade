...
void quicksort(Pokemon *pokemons[], int esq, int dir) {
  if (esq < dir) {
    int i = esq, j = dir;
    ...
    quicksort(pokemons, esq, j);
    quicksort(pokemons, i, dir);
  }
}
...
{   
    ...
    FILE *log_file = fopen("851568_quicksort.txt", "w");
    if (log_file) {
